...
RUN python3 -m pip install -r /rtkbase/web_app/requirements.txt --extra-index-url https://www.piwheels.org/simple
...
CMD ["/rtkbase/docker/rtkbase/entrypoint.sh"]

# example : docker run --rm -it --network=host --device=/dev/ttyACM0 -v /run/udev:/run/udev:ro rtkbase:1.0.0
# -v /run/udev:/run/udev:ro allow to access udev database to detect device on usb
